...
function Tree(container, options) {
  ...
  Object.defineProperties(this, {
	...
	selectedStatusNodes: { //DAN lo creo
		get: function get() {
			var nodes = [];
			var nodesById = this.nodesById;

			for (var id in nodesById) {
				if (nodesById.hasOwnProperty(id) && (nodesById[id].status === 1 || nodesById[id].status === 2 || nodesById[id].status === 0)) {
					var node = Object.assign({}, nodesById[id]);
					delete node.parent;
					delete node.children;
					nodes.push(node);
...
Tree.prototype.onItemClick = function (id) {
  ...
  onChange && onChange.call(this);
	...
	//alert("Dio clic en: " + id + "estado: " + node.status);
	this.getValues();
};
...
Tree.createLiEle = function (node, closed) {
  var li = document.createElement('li');
	...
	//li.setAttribute("status", node.status);
